...
public class UpdateParticipantsCommandHandler : IRequestHandler<UpdateParticipantsCommand, bool>
{
    ...
    public async Task<bool> Handle(UpdateParticipantsCommand request, CancellationToken cancellationToken)
    {
        ...
        await _openBankingDbContext.Participants.AddRangeAsync(organisations, cancellationToken: cancellationToken);
        await _openBankingDbContext.SaveChangesAsync(cancellationToken: cancellationToken);
        return true;
    }
}